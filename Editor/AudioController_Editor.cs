...
public class AudioController_Editor : EditorEx
{
    ...
    AudioCategory currentCategory
    {
        get
        {
            if (currentCategoryIndex < 0 || AC.AudioCategories == null ||
                currentCategoryIndex >= AC.AudioCategories.Length)
            {
                return null;
            }
    ...
    AudioItem currentItem
    {
        get
        {
            ...
            if (currentItemIndex < 0 || curCategory.AudioItems == null ||
                currentItemIndex >= curCategory.AudioItems.Length)
            {
                return null;
            }
    ...
    public override void OnInspectorGUI()
    {
            ...
            lastSubItemIndex != currentSubitemIndex)
        {
            GUIUtility.keyboardControl =
                0; // workaround for Unity weirdness not changing the value of a focused GUI element when changing a category/item
        ...
        if (globalFoldout = EditorGUILayout.Foldout(globalFoldout, "Global Audio Settings", foldoutStyle))
        {
            ...
            bool changed = EditBool(ref currentlyAdditionalController, "Additional Audio Controller",
            ...
            EditBool(ref AC.Persistent, "Persist Scene Loading",
                "A non-persisting AudioController will get destroyed when loading the next scene.");
            ...
            EditBool(ref AC.EqualPowerCrossfade, "Equal-power crossfade",
                "Unfortunatly not 100% correct due to unknown volume formulas used by Unity");
        }
        ...
        if (musicFoldout = EditorGUILayout.Foldout(musicFoldout, "Music Settings", foldoutStyle))
        {
            EditBool(ref AC.specifyCrossFadeInAndOutSeperately, "Separate crossfade in/out",
                "Allows to specify a separate fade-in and out value for all music");
            if (AC.specifyCrossFadeInAndOutSeperately)
            {
                float v_in = AC.musicCrossFadeTime_In;
                EditFloat(ref v_in, "   Music Crossfade-in Time", "sec");
                AC.musicCrossFadeTime_In = v_in;

                float v_out = AC.musicCrossFadeTime_Out;
                EditFloat(ref v_out, "   Music Crossfade-out Time", "sec");
        ...
        if (playlistFoldout = EditorGUILayout.Foldout(playlistFoldout, "Playlist Settings", foldoutStyle))
        {
            ...
            currentPlaylistIndex = Popup("Playlist", currentPlaylistIndex, playListNames,
        ...
        if (categoryFoldout = EditorGUILayout.Foldout(categoryFoldout, "Category Settings", foldoutStyle))
        {
            ...
            if (curCat != null)
            {
                ...
                EditString(ref curCat.Name, "Name",
                    curCat.Name == _nameForNewCategoryEntry ? textAttentionStyle : null);
                ...
                var catList =
                    _GenerateCategoryListIncludingNone(out selectedParentCategoryIndex, curCat.parentCategory);
                ...
                if (itemFoldout = EditorGUILayout.Foldout(itemFoldout, "Audio Item Settings", foldoutStyle))
                {
                    ...
                    int newItemIndex = PopupWithStyle("Item", currentItemIndex, itemNames, popupStyleColored);
                    ...
                    bool changeScroll = currentItemIndex != newItemIndex;
                    if (AudioItem_Editor.Instance != null)
                    {
                        AudioItem_Editor.Invoke(currentCategory, itemNames, newItemIndex, str =>
                        {
                            currentItemIndex = Array.IndexOf(itemNames, str);
                        });
                    }

                    if (GUILayout.Button("F", GUILayout.Width(30)) && itemCount > 0)
                    {
                        AudioItem_Editor.Invoke(currentCategory, itemNames, newItemIndex, str =>
                        {
                            currentItemIndex = Array.IndexOf(itemNames, str);
                    ...
                    if (curItem != null)
                    {
                        ...
                        {
                            EditInt(ref curItem.loopSequenceCount, "   Stop after subitems", "",
                            ...
                            EditFloat(ref curItem.loopSequenceOverlap, "   Overlap", "sec",
                            ...
                            EditFloat(ref curItem.loopSequenceRandomDelay, "   Random Delay", "sec",
                            ...
                            EditFloat01(ref curItem.loopSequenceRandomVolume, "   Random Volume", "±%",
                            ...
                            EditFloat(ref curItem.loopSequenceRandomPitch, "   Random Pitch", "±semitone",
                        ...
                        curItem.SubItemPickMode = (AudioPickSubItemMode) EnumPopup("Pick Subitem Mode",
                            curItem.SubItemPickMode,
                        ...
                        if (subitemFoldout =
                            EditorGUILayout.Foldout(subitemFoldout, "Audio Sub-Item Settings", foldoutStyle))
                        {
                            EditorGUILayout.BeginHorizontal();
                            if (GUILayout.Button("Add selected audio clips", EditorStyles.miniButton))
                            {
                            ...
                            currentSubitemIndex = PopupWithStyle("SubItem", currentSubitemIndex, GetSubitemNames(),
                                popupStyleColored);
                            ...
                            if (GUILayout.Button("+", GUILayout.Width(30)))
                            {
                                ...
                                if (subItemCount > 0)
                                {
                                    ...
                                    {
                                        lastEntryIsNew =
                                            curItem.subItems[currentSubitemIndex].ItemModeAudioID == null ||
        ...
        EditorGUILayout.Space();
        GUILayout.Label(
            string.Format("----- ClockStone Audio Toolkit v{0} -----  ", AudioController.AUDIO_TOOLKIT_VERSION),
            centeredTextStyle);
    ...
    private string[] _GenerateCategoryListIncludingNone(out int selectedParentCategoryIndex,
        AudioCategory selectedAudioCategory)
    {
        string[] names;
    ...
    private void _DisplaySubItem_Clip(AudioSubItem subItem, int subItemCount, AudioItem curItem)
    {
        ...
        if (subItem != null)
        {
            ...
            if (_IsRandomItemMode(curItem.SubItemPickMode))
            {
                ...
                EditBool(ref subItem.DisableOtherSubitems, "Disable Other Subitmes",
            ...
            EditBool(ref subItem.RandomStartPosition, "Random Start Position",
                "Starts playing at a random position. Useful when looping.");
        }
    ...
    private string[] GetSubitemNames()
    {
        ...
        {
            AudioSubItemType subitemType =
                curItem.subItems[i] != null ? curItem.subItems[i].SubItemType : AudioSubItemType.Clip;
            ...
            else
            {
                names[i] = string.Format("CLIP {0}: {1}", i,
                    (curItem.subItems[i] != null
                        ? curItem.subItems[i].Clip
                            ? curItem.subItems[i].Clip.name
                            : "*unset*"
                        : ""));
            }
        }
